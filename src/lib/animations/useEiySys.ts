...
// EiySys reveal — header rises (kicker → title words → sub), then the service
// cards settle up in a stagger. Once on enter; reduced motion returns early
// (renders settled). Mirrors useServices/useCases.
export function useEiySys(scope: RefObject<HTMLElement | null>, s: Styles) {
  useGSAP(
    () => {
